...
public class LoggerNativeBridge implements ILogger {
    ...
    public void loggerInit(String cachePath,
        ...
        try {
            ...
            handler.post(() -> {
                // 初始化完成后清理错误记录，允许重新上报
                handleLoggerStatus(code, MGLoggerStatus.MGLOGGER_INIT_STATUS);
            });
//            loggerStatusCode(MGLoggerStatus.MGLOGGER_INIT_STATUS, code);
    ...
    private void loggerStatusCode(String cmd, int code) {
        ...
        if (code == MGLoggerStatus.MGLOGGER_OK && cmd.equals(MGLoggerStatus.MGLOGGER_INIT_STATUS)) {
            // 初始化成功
        } else if (code == MGLoggerStatus.MGLOGGER_OK && cmd.equals(MGLoggerStatus.MGLOGGER_OPEN_STATUS)) {
        } else if (code != MGLoggerStatus.MGLOGGER_OK) {
