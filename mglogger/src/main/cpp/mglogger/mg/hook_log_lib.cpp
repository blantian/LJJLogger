...
static int hook_log_print(int prio, const char *tag, const char *fmt, ...) {
    ...
    if (orig_log_print) {
        orig_log_print(ANDROID_LOG_DEBUG,
                       "HookLoglib",
                       "hook_log_print ret=%d, prio=%d, tag=%s",
...
static int hook_log_buf_write(int bufID, int prio, const char *tag, const char *text) {
    if (text) {
        ...
        if (orig_log_print) {
            orig_log_print(ANDROID_LOG_DEBUG,
                           "HookLoglib",
                           "hook_log_buf_write ret=%d, prio=%d, tag=%s",
                           cw_ret, prio, tag ? tag : "(null)");
        }
    }
...
static void hook_log_assert(const char *cond, const char *tag, const char *fmt, ...) {
    ...
    char logBuf[1280];
    snprintf(logBuf, sizeof(logBuf), "[cond:%s][%s] %s", cond ? cond : "(null)",
             tag ? tag : "(null)", msgBuf);
...
int hook_log() {
    ...
    xhook_register(".*\\.so$", "__android_log_write", (void *) hook_log_write,
                   (void **) &orig_log_write);
    xhook_register(".*\\.so$", "__android_log_print", (void *) hook_log_print,
                   (void **) &orig_log_print);
    xhook_register(".*\\.so$", "__android_log_vprint", (void *) hook_log_vprint,
                   (void **) &orig_log_vprint);
    xhook_register(".*\\.so$", "__android_log_assert", (void *) hook_log_assert,
                   (void **) &orig_log_assert);
    xhook_register(".*\\.so$", "__android_log_buf_write", (void *) hook_log_buf_write,
